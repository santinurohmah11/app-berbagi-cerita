...
export default class HomePage {
  ...
  async afterRender() {
    // 🔹 Pastikan elemen map sudah tersedia
    await this._waitForMapElement();

    // 🔹 Inisialisasi map dengan aman
    this._initMapSafe();

    // 🔹 Ambil data & tampilkan marker
    await this._fetchStories();
    this._renderList();
    this._renderMarkers();

    // 🔹 Aktifkan klik daftar ↔ marker
    this._setupInteraction();

    // 🔹 Pastikan ukuran peta disesuaikan
    setTimeout(() => {
  ...
  _initMapSafe() {
    ...
    if (mapContainer && mapContainer._leaflet_id) {
      // 🔥 Jika sudah ada, hapus instance lama dulu
